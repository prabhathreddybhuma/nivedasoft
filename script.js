...
function renderCourseCatalog() {
    ...
    if (!courseGrid || typeof courseData === 'undefined') return;
    ...
    courseData.forEach(course => {
        ...
        if (cat.includes('hana')) icon = 'fa-database';
        else if (cat.includes('fiori') || cat.includes('ui')) icon = 'fa-mobile-alt';
        else if (cat.includes('finance')) icon = 'fa-chart-pie';
        else if (cat.includes('analytics')) icon = 'fa-chart-line';
        else if (cat.includes('security') || cat.includes('governance')) icon = 'fa-shield-alt';
        else if (cat.includes('cloud')) icon = 'fa-cloud';
        else if (cat.includes('warehouse')) icon = 'fa-warehouse';

        // Create card HTML
        const card = document.createElement('div');
        card.className = 'course-card';
        card.setAttribute('data-level', (course.level || 'Intermediate').toLowerCase());
        ...
        card.innerHTML = `
            ...
            <a href="contact.html?course=${encodeURIComponent(course.code)}" class="course-btn">Enroll Now</a>
        `;
...
function initCourseFilters() {
    ...
    function applyFilters(category, searchText) {
        ...
        cards.forEach(card => {
            ...
            const cat = card.getAttribute('data-category'); // e.g. 'finance'
            ...
            if (category !== 'all') {
                if (category === 'certification') {
                    // Logic for certification if data attribute exists, or check text
                    matchesCategory = card.querySelector('.course-certification') !== null;
                } else {
                    // Check both level and category for flexibility
                    matchesCategory = level === category || cat === category;
                }
            }
...
document.addEventListener('DOMContentLoaded', () => {
    renderCourseCatalog(); // Render courses on load
    initCourseFilters(); // Initialize filters
